...
function mobile_xhtml_siteFooter( $rightblock = -1, $custom = '' )
{
    ...
    // ホーム
    $footer->set_var( 'mn_tohome', '[emoji:134]<a href="'. $_CONF['site_url'] .
    ...
    if (!empty ($_USER['uid']) && ($_USER['uid'] > 1)) {
        	$footer->set_var( 'mn_login_or_logout',
						  "[emoji:$emoji]" . '<a href="'. $_CONF['site_url'] .
    ...
    // 記事投稿
    $footer->set_var( 'mn_submit', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
	...
	if (DB_numRows($temp) == 1) {
	    $footer->set_var( 'mn_forum', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
    	                  '/forum/index.php" accesskey="' . $akey . '">' . "论坛</a>" );
		$akey ++;
		$emoji ++;
	}

   // ダウンロード プラグイン メニュー
	$temp = DB_query("SELECT 1 AS cnt FROM {$_TABLES['plugins']} WHERE (pi_name = 'filemgmt') AND (pi_enabled = '1')");
	if (DB_numRows($temp) == 1) {
	    $footer->set_var( 'mn_filemgmt', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
    	                  '/filemgmt/index.php" accesskey="' . $akey . '">' . "下载</a>" );
		$akey ++;
		$emoji ++;
	}

   // メディアギャラリー　プラグイン　メニュー
	$temp = DB_query("SELECT 1 AS cnt FROM {$_TABLES['plugins']} WHERE (pi_name = 'mediagallery') AND (pi_enabled = '1')");
	if (DB_numRows($temp) == 1) {
	    $footer->set_var( 'mn_mediagallery', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
    	                  '/mediagallery/index.php" accesskey="' . $akey . '">' . "媒体库</a>" );
		$akey ++;
		$emoji ++;
	}

    // 記事一覧
    $footer->set_var( 'mn_directory', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
                      '/directory.php" accesskey="' . $akey . '">' . $LANG01['117'] . '</a>' );
	$akey ++;
	$emoji ++;
	
    // 検索
    $footer->set_var( 'mn_search', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
                      '/search.php" accesskey="' . $akey . '">' . $LANG01['75'] . '</a>' );
	$akey ++;
	$emoji ++;
	
    // ブロック
    $footer->set_var( 'mn_block', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
                      '/mobileblocks.php" accesskey="' . $akey . '">子菜单</a>' );
	$akey ++;
	$emoji ++;
	
    if (!empty ($_USER['uid']) && ($_USER['uid'] > 1)) {
    // マイアカウント
        $footer->set_var( 'mn_myaccount', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
    ...
    // 新規登録
        	$footer->set_var( 'mn_myaccount', "[emoji:$emoji]" . '<a href="' . $_CONF['site_url'] .
                      '/users.php?mode=new" accesskey="' . $akey . '">注册帐户</a>' );
		$akey ++;
		$emoji ++;
    }
